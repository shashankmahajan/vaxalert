...
def filter_slots_with_availability(
    ...
    Args:
        ...
        pincodes_of_interest (List[str]): List of pincodes to monitor
    ...
    """
    availability_data = availability_data.loc[availability_data['available_capacity'] > 0]
    availability_data = availability_data.loc[availability_data.min_age_limit.isin(min_age_limit_filter)]
    availability_data = availability_data.loc[availability_data.pincode.isin(pincodes_of_interest)]
...
def generate_available_appointments_message(
    ...
    dates_array = pd.date_range(
        today, periods=num_days_to_lookup).to_pydatetime().tolist()
    dates_array = list(map(lambda x: x.date(), dates_array))

    appointments_data_array = []

    for date in dates_array:
        response = get_vaccine_availability_data_by_district_date(
            district_id, date)
    ...
    availability_data = filter_slots_with_availability(
        appointments_data,
        pincodes_of_interest,
        min_age_limit_filter=min_age_limit_filter
    )
    ...
    availability_data.drop_duplicates(inplace=True)
    messages = []

    for index, row in availability_data.iterrows():
        sub_msg = f"{int(row['available_capacity'])} slots available at:\n"
        sub_msg += f"Name: {row['name']}\n"
        sub_msg += f"Date: {row['date']}\n"
        sub_msg += f"Pincode: {row['pincode']}\n"
        sub_msg += f"Vaccine: {row['vaccine']}\n"
        sub_msg += f"Age: {row['min_age_limit']}\n"
...
def run_vaccine_slot_alert():
    ...
    for message in messages:
        post_message_to_telegram(message)
